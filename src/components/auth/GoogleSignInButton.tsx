...
export default function GoogleSignInButton() {
    ...
    // Shared logic to handle user document creation/check after successful sign-in
    const handleUserResult = useCallback(async (user: User) => {
        if (!db) {
        ...
        try {
            ...
            if (!userDocSnap.exists()) {
                ...
                try {
                    ...
                    const message = (writeError instanceof Error) ? writeError.message : (t?.auth?.errors?.unknownGoogleSignInError || 'Unknown error');
                    setError(`${t?.auth?.errors?.profileSaveFailedPrefix || 'Profile save failed: '}${message}`);
                }
            } else {
            ...
            const message = (firestoreCheckError instanceof Error) ? firestoreCheckError.message : (t?.auth?.errors?.unknownGoogleSignInError || 'Unknown error');
            setError(`${t?.auth?.errors?.profileCheckSaveFailedPrefix || 'Profile check failed: '}${message}`);
        }
    }, [t]); // Only t is needed; db is a module-level constant

    // Check for redirect result on mount (for mobile flow)
    useEffect(() => {
        if (auth) {
            getRedirectResult(auth)
                .then((result) => {
                    if (result) {
                        console.log('Google Sign-In successful (Redirect):', result.user.uid);
                        handleUserResult(result.user);
                    }
                })
                .catch((authError) => {
                    console.error("Google Sign-In error (Redirect):", authError);
                    if (t) {
                        setError(`${t.auth.errors.googleSignInFailedPrefix}${authError.message}`);
                    }
                });
        }
    }, [t, handleUserResult]); // Add dependencies
    ...
    const handleGoogleSignIn = async () => {
        ...
        try {
            // Try Popup first (Desktop preferred)
            const result = await signInWithPopup(auth, provider);
            const user = result.user;
            console.log('Google Sign-In successful (Popup):', user.uid);
            await handleUserResult(user);
        } catch (authError: unknown) {
            console.error("Google Sign-In error (Popup):", authError);

            if (authError instanceof FirebaseError) {
                // FALLBACK: If popup is blocked (common on iPhone 7/iOS 15), try Redirect
                if (authError.code === 'auth/popup-blocked' || authError.code === 'auth/popup-closed-by-user') {
                    ...
                    try {
                        ...
                        // Note: Code execution stops here as page redirects
                        return;
                    } catch (redirectError) {
                        console.error("Redirect failed:", redirectError);
                        setError(`${t.auth.errors.googleSignInFailedPrefix}${(redirectError as Error).message}`);
                    }
